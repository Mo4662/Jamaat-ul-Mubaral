...
export default function Events() {
  const rows = [
    { date: '2025-10-12', title: 'Community Clean-up', location: 'Central Park' },
    { date: '2025-11-05', title: 'Food Drive', location: 'Community Center' },
    { date: '2025-12-02', title: 'Mentorship Night', location: 'Library Hall' },
  ]
  return (
    <main className="section">
      <div className="container">
          ...
          <tbody>
            {rows.map((r,i)=> (
              <tr key={i}>
                ...
                <td>{r.title}</td>
                <td>{r.location}</td>
                <td><NavLink className="button ghost" to={`/events#${i}`}>RSVP</NavLink></td>
              </tr>
            ))}
          </tbody>
        </table>
